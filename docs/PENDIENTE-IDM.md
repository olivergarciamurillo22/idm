...
| 2 | **Export Proveedores** (código, razón social, CIF, correo) | Identificar al proveedor de cada documento por CIF; correo para enviar pedidos; claves reales del registro | `equivalencias/proveedores` (rellenar `codigo_siddex` y `cif` de los 6 grandes), `pedidos/enviar` | **Crítica** | **Evidencia parcial (fotos 23/09):** códigos Siddex y CIF de dos proveedores vistos en pantalla y papel → en `datos/proveedores_conocidos.csv` (fuera de git). Falta el export completo con correos |
...
| 13 | **Códigos de escandallo A4R L / E / LE** (y si existen como producto en Siddex o son base + opciones) | Calcular necesidades de todas las variantes del planning | `datos/mapa_productos.csv` (ejemplo versionado: `necesidades/mapa_productos.ejemplo.csv`) | **Crítica** (hito 2) | Pendiente (Fernandillo) |
| 14 | **Mapa nombre de planning → código Siddex** para todos los productos del mes | Sin él, el cálculo del mes salta filas | `datos/mapa_productos.csv` (ejemplo versionado: `necesidades/mapa_productos.ejemplo.csv`) | **Crítica** (hito 2) | Pendiente (Fernandillo) |
