...
CABECERAS = ["Proveedor (código Siddex)", "Nuestro pedido", "Fecha", "Artículo", "Descripción", "Cantidad",
             "Unidad", "Precio", "Dto %"]
...
def exportar(pedidos: list[Pedido], proveedores: dict[str, Proveedor], ruta: Path) -> Path:
    ...
    for pedido in pedidos:
        ws = libro.create_sheet(pedido.numero[:31])
        ws.append(CABECERAS)
        for c in ws[1]:
            c.font = Font(bold=True)
        prov = proveedores.get(pedido.proveedor)
        for li in pedido.lineas:
            ws.append([prov.codigo_siddex if prov else pedido.proveedor, pedido.numero,
                       pedido.fecha.isoformat() if pedido.fecha else "", li.codigo_idm, li.descripcion,
                       float(li.cantidad), li.unidad, float(li.precio_bruto), float(li.descuento_pct)])
