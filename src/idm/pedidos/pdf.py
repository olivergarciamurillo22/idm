...
def nombre_fichero(pedido: Pedido) -> str:
    return f"PEDIDO_{nombre_seguro(pedido.numero)}_{pedido.proveedor}.pdf"
...
def generar_pdf(pedido: Pedido, proveedor: Proveedor | None, empresa: dict[str, str], carpeta: Path) -> Path:
    ...
    cuerpo = [
        ...
        Paragraph(
            f"Proveedor: <b>{proveedor.nombre if proveedor else pedido.proveedor}</b>"
            + (f" · CIF {proveedor.cif}" if proveedor and proveedor.cif else ""),
            estilos["Normal"],
        ),
        Spacer(1, 6 * mm),
    ]
    ...
    for li in pedido.lineas:
        filas.append(
            [
                li.codigo_idm,
                li.descripcion,
                formatear(li.cantidad),
                li.unidad,
                formatear(li.precio_bruto),
                formatear(li.descuento_pct),
                formatear(li.importe),
            ]
        )
