...
def test_hook_pre_commit_bloquea_documento_real(raiz, tmp_path):
    ...
    subprocess.run([GIT, "add", "-f", "docs/albaran_real.pdf", "notas.md"], cwd=repo, check=True)
    r = subprocess.run(["sh", str(hook)], cwd=repo, capture_output=True, text=True)
    assert r.returncode == 1 and "BLOQUEADO" in r.stdout
    subprocess.run([GIT, "rm", "-q", "--cached", "docs/albaran_real.pdf"], cwd=repo, check=True)
    r = subprocess.run(["sh", str(hook)], cwd=repo, capture_output=True, text=True)
